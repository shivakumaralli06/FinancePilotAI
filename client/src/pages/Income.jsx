...
const Income = () => {
  ...
  const fetchIncomes = async () => {
    ...
    try {
      ...
      addToast('Failed to fetch income records', 'error');
    } finally {
      setLoading(false);
    }
  };
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!formData.source || !formData.amount || parseFloat(formData.amount) <= 0) {
      addToast('Please enter a valid income source and positive amount.', 'warning');
      return;
    }
    ...
    try {
      if (editingItem) {
        await incomeService.updateIncome(editingItem.id, formData);
        addToast('Income record updated successfully!', 'success');
      } else {
        await incomeService.createIncome(formData);
        addToast('Income record added successfully!', 'success');
      }
  ...
  const filteredIncomes = incomes.filter(item => {
    ...
    const matchesCategory = categoryFilter === 'ALL' || item.source === categoryFilter;
  ...
  const totalIncomeAmount = incomes.reduce((sum, item) => sum + parseFloat(item.amount || 0), 0);

  if (loading) {
    return <Loader text="Loading income telemetry..." />;
  }
  ...
  return (
    <div className="space-y-6">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
        <div>
          <h1 className="text-2xl font-bold text-slate-900 dark:text-white flex items-center gap-2">
            <TrendingUp className="w-6 h-6 text-emerald-500" /> Income Manager
          </h1>
          <p className="text-sm text-slate-500 dark:text-slate-400 mt-1">
            Track and classify all your incoming revenue streams
          </p>
        </div>
      ...
      <div className="glass-card p-6 flex items-center justify-between">
        <div>
          <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">Cumulative Income</div>
          <div className="text-3xl font-extrabold text-emerald-500 mt-1">{formatCurrency(totalIncomeAmount)}</div>
        </div>
        <div className="text-right">
          <div className="text-xs font-semibold uppercase tracking-wider text-slate-500">Total Entries</div>
          <div className="text-2xl font-bold text-slate-900 dark:text-white mt-1">{incomes.length}</div>
        </div>
      </div>
      ...
      <Modal
        ...
        title={editingItem ? 'Edit Income Entry' : 'Add New Income Entry'}
      >
